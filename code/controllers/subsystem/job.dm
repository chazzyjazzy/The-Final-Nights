...
/datum/controller/subsystem/job/proc/EquipRank(mob/M, rank, joined_late = FALSE)
	...
	if(job)
		...
		if((iskindred(human) && human.clan) || iscathayan(human) || isgarou(human) || iszombie(human))
			...
			if(job.title != "Prince")
				to_chat(M, "<span class='notice' style='color:red;'><b>The Sabbat rule the city. You should obey them, their laws and the Archbishop, at least in public.</b></span>")
			if(job.title == "Chantry Archivist")
				to_chat(M, span_notice("<b>As a member of the Chantry, you are part of the Sabbat inversion of the Tremere Pyramid and are blood bonded to the Bishop. Always be loyal.</b>"))
