...
/datum/stock_market/proc/initialize_sectors()
	sectors = list(
		SECTOR_TECH = new /datum/stock_sector(SECTOR_TECH, 1.8, 0.7),
		SECTOR_HEALTHCARE = new /datum/stock_sector(SECTOR_HEALTHCARE, 1.4, 0.6),
		SECTOR_COMMODITIES = new /datum/stock_sector(SECTOR_COMMODITIES, 2.2, 0.8),
		SECTOR_LOGISTICS = new /datum/stock_sector(SECTOR_LOGISTICS, 1.6, 0.5),
		SECTOR_CORPORATE = new /datum/stock_sector(SECTOR_CORPORATE, 1.3, 0.6),
		SECTOR_FINANCE = new /datum/stock_sector(SECTOR_FINANCE, 2.0, 0.9)
	)
...
/datum/stock_sector/proc/update_performance(market_sentiment)
	...
	var/sentiment_effect = market_sentiment * 0.3
	var/random_effect = (rand(-20, 20) / 100.0) * volatility
	var/correlation_decay = performance * -0.1 // Mean reversion

	performance = sentiment_effect + random_effect + correlation_decay
	performance = max(-0.5, min(0.5, performance)) // Cap extreme values
...
/datum/stock/proc/update_price()
	...
	var/base_change = (rand(-100, 100) / 100.0) * VOLATILITY_MULTIPLIER

	// Sector influence
	var/sector_influence = sector_data.performance * sector_data.correlation

	// Market sentiment influence
	var/sentiment_influence = market_ref.market_sentiment * 0.2

	// Momentum influence (trending stocks continue trending)
	var/momentum_influence = momentum * 0.1
	momentum += (base_change * 0.1) // Update momentum
	momentum *= 0.9 // Decay momentum over time

	// Combine all factors
	var/total_change = (base_change + sector_influence + sentiment_influence + momentum_influence) / 100.0
